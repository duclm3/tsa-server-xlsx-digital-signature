 ...
 */
public class XAdESXLSignatureFacet1 implements SignatureFacet {

    private static final Logger LOG = LogManager.getLogger(XAdESXLSignatureFacet1.class);

    private final CertificateFactory certificateFactory;

    public XAdESXLSignatureFacet1() {
        try {
    ...
    public void postSign(SignatureInfo signatureInfo, Document document) throws MarshalException {
        ...
        if (chainSize > 1) {
            for (X509Certificate cert : certChain.subList(1, chainSize)) {
                ...
                setCertID(certId, signatureConfig, false, cert);
            }
        }
        ...
        if (revocationData.hasCRLs()) {
            ...
            for (byte[] encodedCrl : revocationData.getCRLs()) {
                ...
                setDigestAlgAndValue(digestAlgAndValue, encodedCrl, signatureConfig.getDigestAlgo());
            }
        }
        if (revocationData.hasOCSPs()) {
            ...
            for (byte[] ocsp : revocationData.getOCSPs()) {
                try {
                    ...
                    setDigestAlgAndValue(digestAlgAndValue, ocsp, signatureConfig.getDigestAlgo());
